...
const rawStatistics = ref<import('@/services/webhook').WebhookStatistics | null>(null)

// 事件类型
const eventTypes = [
  { value: 'push', label: '镜像推送', description: '当镜像被推送到仓库时触发', icon: '📤' },
  { value: 'pull', label: '镜像拉取', description: '当镜像被拉取时触发', icon: '📥' },
  { value: 'delete', label: '镜像删除', description: '当镜像被删除时触发', icon: '🗑️' },
  { value: 'scan', label: '扫描完成', description: '当安全扫描任务完成时触发', icon: '🔍' },
  { value: 'scan_fail', label: '扫描失败', description: '当安全扫描任务失败时触发', icon: '❌' },
]
...
const statistics = computed(() => ({
  ...
  active: rawStatistics.value?.activeWebhooks ?? webhooks.value.filter((w) => w.isActive).length,
  totalTriggers:
...
async function handleSaveWebhook() {
  ...
  try {
    ...
    const index = webhooks.value.findIndex((w) => w.webhookId === selectedWebhook.value!.webhookId)
    if (index !== -1) {
...
async function toggleWebhookStatus(webhook: Webhook) {
  try {
    await webhookApi.updateWebhook(webhook.webhookId, { isActive: !webhook.isActive })
...
async function submitTest() {
  ...
  try {
    const result = await webhookApi.testWebhook(selectedWebhook.value.webhookId, testPayload.value)
  ...
  <div class="webhook-page">
    <div class="page-header">
      <div class="header-left">
        ...
        <p class="page-subtitle">配置外部系统的事件通知，实现与CI/CD、监控等系统的集成</p>
      </div>
    ...
    <div class="stats-row">
      ...
      <div class="stat-card">
        <div class="stat-icon triggers">
          <svg viewBox="0 0 24 24" width="24" height="24">
            <path fill="currentColor" d="M13 2.05v2.02c3.95.49 7 3.85 7 7.93 0 3.21-1.92 6-4.72 7.28L13 17v5h5l-1.22-1.22C19.91 19.07 22 15.76 22 12c0-5.18-3.95-9.45-9-9.95zM11 2.05C5.95 2.55 2 6.82 2 12c0 3.76 2.09 7.07 5.22 8.78L6 22h5v-5l-2.28 2.28C6.92 18 5 15.21 5 12c0-4.08 3.05-7.44 7-7.93V2.05z"/>
          </svg>
        </div>
        <div class="stat-content">
          <div class="stat-value">{{ statistics.totalTriggers }}</div>
    ...
    <div class="webhook-list">
      <CypCard v-for="webhook in webhooks" :key="webhook.webhookId" class="webhook-card">
        <template #header>
        ...
        <div class="webhook-content">
          ...
          <div class="content-row stats">
            <div class="mini-stat">
              <span class="mini-value success">{{ webhook.successCount || 0 }}</span>
              <span class="mini-label">成功</span>
            </div>
            <div class="mini-stat">
              <span class="mini-value danger">{{ webhook.failedCount || 0 }}</span>
              <span class="mini-label">失败</span>
            </div>
            <div class="mini-stat" v-if="webhook.lastTriggeredAt">
              <span class="mini-value">{{ formatDate(webhook.lastTriggeredAt) }}</span>
        ...
        <template #footer>
          <div class="webhook-actions">
            <CypButton size="small" @click="openDetailDialog(webhook)">详情</CypButton>
            <CypButton size="small" @click="openTestDialog(webhook)">测试</CypButton>
            <CypButton size="small" @click="openEditDialog(webhook)">编辑</CypButton>
            <CypButton size="small" type="danger" @click="handleDeleteWebhook(webhook.webhookId)">
              删除
            </CypButton>
          </div>
        </template>
      ...
      <div v-if="webhooks.length === 0" class="empty-state">
        ...
        <CypButton type="primary" @click="showCreateDialog = true">创建Webhook</CypButton>
      </div>
    </div>
    ...
    >
      <div class="dialog-form">
        ...
        <div class="form-group">
          <label>回调URL *</label>
          <CypInput v-model="newWebhook.url" placeholder="https://example.com/webhook" />
        </div>
      ...
      <template #footer>
        ...
        <CypButton type="primary" @click="handleCreateWebhook">创建</CypButton>
      </template>
    </CypDialog>
    ...
    >
      <div v-if="selectedWebhook" class="dialog-form">
        ...
        <div class="form-group">
          <label>回调URL *</label>
          <CypInput v-model="selectedWebhook.url" placeholder="https://example.com/webhook" />
        </div>
    ...
    >
      <div class="dialog-form">
        <div class="test-info" v-if="selectedWebhook">
        ...
        <div class="form-group">
          ...
          <CypSelect
            ...
            :options="eventTypes.map(e => ({ value: e.value, label: e.label }))"
          />
        </div>
      ...
      <template #footer>
        ...
        <CypButton type="primary" :loading="isTesting" @click="submitTest">发送测试</CypButton>
      </template>
    </CypDialog>
        ...
        <div class="detail-section">
          ...
          <div class="stats-cards">
            <div class="stats-card success">
              <span class="stats-number">{{ selectedWebhook.successCount || 0 }}</span>
              <span class="stats-label">成功</span>
            </div>
            <div class="stats-card danger">
              <span class="stats-number">{{ selectedWebhook.failedCount || 0 }}</span>
              <span class="stats-label">失败</span>
            </div>
            <div class="stats-card" v-if="selectedWebhook.lastTriggeredAt">
              <span class="stats-number">{{ formatDate(selectedWebhook.lastTriggeredAt) }}</span>
    ...
    >
      <div v-if="webhookToDelete" class="confirm-content">
        <p>
          确定要删除 Webhook
          "<strong>{{ webhookToDelete.name }}</strong>" 吗？
        </p>
...
.stat-icon {
  ...
  &.total { background: #e0e7ff; color: #6366f1; }
  &.active { background: #dcfce7; color: #22c55e; }
  &.triggers { background: #fef3c7; color: #f59e0b; }
}
