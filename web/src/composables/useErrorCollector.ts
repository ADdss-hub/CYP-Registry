...
const stats = computed(() => {
  ...
  const successes = errorRecords.value.filter(
    (r) => r.type === "success",
  ).length;
...
function getRecentErrors(limit: number = 10): ErrorRecord[] {
  return errorRecords.value.filter((r) => r.type === "error").slice(0, limit);
}
...
function getRecentSuccesses(limit: number = 10): ErrorRecord[] {
  return errorRecords.value.filter((r) => r.type === "success").slice(0, limit);
}
