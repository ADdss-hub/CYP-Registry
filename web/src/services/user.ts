...
function mapPATToAccessToken(
  dto: PATResponseDto | CreatePATResponseDto,
  tokenValue?: string,
): AccessToken {
  // 后端使用 9999-12-31T23:59:59Z 表示“永不过期”，前端在此处转为语义字段
  const isNeverExpire =
    dto.expires_at && dto.expires_at.startsWith("9999-12-31");
  ...
  return {
    ...
    lastUsedAt:
      "last_used_at" in dto ? (dto.last_used_at ?? undefined) : undefined,
    expiresAt,
    scopes: dto.scopes,
  };
}
...
export const userApi = {
  ...
  getCurrentUser: () => api.get<User>("/v1/users/me") as Promise<User>,
  ...
  // 登出
  logout: () => api.post<void>("/v1/auth/logout") as Promise<void>,

  // 首次部署：一次性获取默认管理员账号信息（用户名 + 密码），用于登录页提示并复制保存
  getDefaultAdminOnce: () =>
    api.get<{ username: string; password: string; created_at?: string }>(
      "/v1/auth/default-admin-once",
    ),
  ...
  getUsers: (params?: { page?: number; pageSize?: number; keyword?: string }) =>
    api.get<PaginatedResponse<User>>("/v1/users", params) as Promise<
      PaginatedResponse<User>
    >,

  // 获取单个用户
  // 注意：api.get 的响应拦截器会返回 payload.data，所以实际返回的是 User
  getUser: (id: string) => api.get<User>(`/v1/users/${id}`) as Promise<User>,
  ...
  createPAT: async (payload: {
    ...
    const body: CreatePATRequestDto = {
      ...
      expire_in:
        payload.expireInDays != null ? payload.expireInDays * 24 * 60 * 60 : -1,
    };
