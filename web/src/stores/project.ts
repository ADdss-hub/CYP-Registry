...
export const useProjectStore = defineStore('project', () => {
  ...
  // 计算属性
  const publicProjects = computed(() => projects.value.filter(p => p.isPublic))
  const myProjects = computed(() => projects.value.filter(p => p.isPublic || true)) // 根据实际权限过滤

  // 方法
  async function fetchProjects(params?: { page?: number; pageSize?: number; keyword?: string; isPublic?: boolean }) {
    ...
    try {
      ...
      const pageSize = data?.pageSize ?? data?.page_size ?? pagination.value.pageSize

      projects.value = Array.isArray(listRaw) ? listRaw.map(mapProjectResponse) : []
  ...
  async function createProject(data: { name: string; description: string; isPublic: boolean; storageQuota?: number }) {
    isLoading.value = true
    error.value = null
    ...
    try {
      ...
      if (project) {
        // 检查是否已存在（避免重复添加）
        const exists = projects.value.find(p => p.id === project.id || p.name === project.name)
        if (!exists) {
      ...
      if (err.code === 20002 || err.message?.includes('已存在') || err.message?.includes('already exists')) {
        const conflictError: any = new Error(err.message || '项目已存在，请使用其他名称')
  ...
  async function updateProject(id: string, data: { name?: string; description?: string; isPublic?: boolean; storageQuota?: number }) {
    isLoading.value = true
    error.value = null
    ...
    try {
      ...
      const updated: any = mapProjectResponse((response as any)?.project ?? (response as any))
  ...
  async function deleteProject(id: string) {
    ...
    try {
      ...
      if (typeof pagination.value.total === 'number' && pagination.value.total > 0) {
  ...
  function updateProjectStats(id: string, stats: { imageCount?: number; storageUsed?: number }) {
