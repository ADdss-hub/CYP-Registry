...
export type NotificationSource = 'scan' | 'project' | 'registry' | 'webhook' | 'system'
...
export const useNotificationStore = defineStore('notification', () => {
  ...
  async function loadFromServer() {
    try {
      const projectPage = await projectApi.getProjects({ page: 1, pageSize: 5 })
      const projectList = ((projectPage as any)?.list ?? (projectPage as any)?.projects ?? []) as Project[]

      const projectNotifications: NotificationItem[] = projectList.map((project) => ({
        id: `project-${project.id}`,
        source: 'project',
        title: '新项目创建',
        message: `项目「${project.name}」已创建${project.isPublic ? '（公开项目）' : ''}`,
        status: 'success',
        createdAt: (project as any).createdAt ?? (project as any).created_at ?? '',
        read: false,
      }))
