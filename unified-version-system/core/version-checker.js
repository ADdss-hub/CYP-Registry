 ...
 * 负责版本格式验证、规则检查、变更类型验证和数字限制检查
 * 
 * 功能特性：
 * - 版本格式验证（SemVer 2.0.0）
 * - 版本号数字限制检查（MAJOR: 0-99, MINOR: 0-50, PATCH: 0-30）
 * - 变更类型验证（Conventional Commits）
 * - 版本递增规则验证
 * - 重复版本检测
 * 
...
class VersionChecker {
  ...
  checkVersionLimits(version) {
    ...
    if (parsed.major > this.limits.major.mandatory) {
      errors.push(`主版本号超出强制限制 (${parsed.major} > ${this.limits.major.mandatory})，必须重新评估版本策略`);
    }
    if (parsed.minor > this.limits.minor.mandatory) {
      errors.push(`次版本号超出强制限制 (${parsed.minor} > ${this.limits.minor.mandatory})，必须递增主版本号 (MAJOR) 并重置次版本号`);
    }
    if (parsed.patch > this.limits.patch.mandatory) {
      errors.push(`修订号超出强制限制 (${parsed.patch} > ${this.limits.patch.mandatory})，必须递增次版本号 (MINOR) 并重置修订号`);
    }

    // 检查警告阈值（SHOULD）
    if (parsed.major > this.limits.major.warning && parsed.major <= this.limits.major.mandatory) {
      warnings.push(`主版本号较大 (${parsed.major} > ${this.limits.major.warning})，建议重新评估版本策略`);
    }
    if (parsed.minor > this.limits.minor.warning && parsed.minor <= this.limits.minor.mandatory) {
      warnings.push(`次版本号较大 (${parsed.minor} > ${this.limits.minor.warning})，建议尽快递增主版本号 (MAJOR)`);
    }
    if (parsed.patch > this.limits.patch.warning && parsed.patch <= this.limits.patch.mandatory) {
      warnings.push(`修订号较大 (${parsed.patch} > ${this.limits.patch.warning})，建议尽快递增次版本号 (MINOR)`);
    }
